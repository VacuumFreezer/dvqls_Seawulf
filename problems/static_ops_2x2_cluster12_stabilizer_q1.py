"""
12-qubit 2x2 distributed benchmark with the first qubit as the block index.
...
with
    |b> = |C_12>
    A = alpha I
        + c_1 K_1 + c_5 K_5 + c_8 K_8 + c_11 K_11
        + gamma X_1,
where
    K_1  = X_1 Z_2,
    K_j  = Z_{j-1} X_j Z_{j+1} for j in {5, 8, 11}.

This choice keeps the system real and commuting, makes the off-diagonal block
sector simple through the boundary index qubit, and avoids the trivial
`x proportional to b` case because `|b>` is not an eigenstate of `X_1`.
"""
...
STABILIZER_SUPPORTS = [(1, 2), (4, 5, 6), (7, 8, 9), (10, 11, 12)]
GLOBAL_TERMS: List[Tuple[float, Dict[int, str], str]] = [
    (ALPHA, {}, "I"),
    (COEFF_K1, {1: "X", 2: "Z"}, "K_1"),
...
COMMUTING_TERM_COEFFS = (COEFF_K1, COEFF_K5, COEFF_K8, COEFF_K11, COEFF_X1)
...
PERTURBATION_SUPPORT_GLOBALS = (2, 3)
...
def _global_pauli_map_to_centralized_word(pauli_map: Dict[int, str]) -> tuple[str, ...]:
    chars = ["I"] * N_TOTAL_QUBITS
    for global_qubit, label in pauli_map.items():
        if global_qubit == INDEX_GLOBAL_QUBIT:
            chars[0] = str(label)
        else:
            chars[GLOBAL_TO_LOCAL[int(global_qubit)] + 1] = str(label)
    return tuple(chars)
...
CENTRALIZED_TERMS = [
    (float(coeff), _global_pauli_map_to_centralized_word(pauli_map))
    for coeff, pauli_map, _label in GLOBAL_TERMS
]

CENTRALIZED_B_PREP_SPEC = {
    "mode": "index_superposition_cluster_pauli_z",
    "index_wire": 0,
    "data_wires": list(range(1, N_TOTAL_QUBITS)),
    "cluster_edges": [[left + 1, right + 1] for left, right in CLUSTER_SCAFFOLD_EDGES_LOCAL],
    "conditional_z_wires": [GLOBAL_TO_LOCAL[g] + 1 for g in _removed_index_neighbors()],
}
...
SYSTEM_2X2 = LinearSystemData(
    ...
    name="cluster12_stabilizer_2x2_q1_index",
    metadata={
        ...
        "centralized_b_prep_spec": dict(CENTRALIZED_B_PREP_SPEC),
        "design_note": (
            "boundary index qubit keeps the removed graph connected and the off-diagonal sector simple; "
            "the X_1 perturbation keeps x nontrivial while preserving a real commuting formulation"
        ),
    },
    max_dense_qubits=16,
)
...
A_CENTRALIZED = np.array(SYSTEM_2X2.get_global_matrix(), dtype=np.complex128)
B_CENTRALIZED = np.array(SYSTEM_2X2.get_global_b_vector(), dtype=np.complex128)

CENTRALIZED_PROBLEMS = {
    "centralized": {
        "name": "cluster12_stabilizer_q1_centralized",
        "n_total_qubits": N_TOTAL_QUBITS,
        "n_index_qubits": 1,
        ...
        "reference_system_key": "2x2",
        "b_prep_spec": dict(CENTRALIZED_B_PREP_SPEC),
    }
}
