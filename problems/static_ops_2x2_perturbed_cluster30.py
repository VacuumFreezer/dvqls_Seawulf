"""
30-qubit 2x2 "perturbed cluster state" benchmark with qubit 1 as the index qubit.

The global system is

    A(eps) = 0.51 I + (0.049 - eps / 10) * sum_{j in S} K_j + eps * Z_30

with S = {1, 4, 7, 10, 13, 16, 19, 22, 25, 28}, K_1 = X_1 Z_2, and
K_j = Z_{j-1} X_j Z_{j+1} for j in S \\ {1}.  The reference vector |b> is
the 30-qubit open-chain cluster state prepared by an RY(pi/2) layer followed
by alternating CZ bonds.  Because all CZ gates commute, the reference state on
the retained 29-qubit half is represented exactly by a single pre-CZ RY layer
and one open-chain CZ scaffold.
"""
...
EPSILON = 0.01
STABILIZER_COEFF = 49.0 / 1000.0 - EPSILON / 10.0

STABILIZER_INDEX_SET = (1, 4, 7, 10, 13, 16, 19, 22, 25, 28)
GLOBAL_TERMS: List[Tuple[float, Dict[int, str], str]] = [
    ...
    (STABILIZER_COEFF, {12: "Z", 13: "X", 14: "Z"}, "K_13"),
    (STABILIZER_COEFF, {15: "Z", 16: "X", 17: "Z"}, "K_16"),
    (STABILIZER_COEFF, {18: "Z", 19: "X", 20: "Z"}, "K_19"),
    (STABILIZER_COEFF, {21: "Z", 22: "X", 23: "Z"}, "K_22"),
    (STABILIZER_COEFF, {24: "Z", 25: "X", 26: "Z"}, "K_25"),
    (STABILIZER_COEFF, {27: "Z", 28: "X", 29: "Z"}, "K_28"),
    (EPSILON, {30: "Z"}, "Z_30"),
]
...
EXACT_LAST_WIRE_ANGLE = float(2.0 * np.arctan2(EXACT_ALPHA - EXACT_BETA, EXACT_ALPHA + EXACT_BETA))
...
SYSTEM_2X2 = LinearSystemData(
    ...
    name="perturbed_cluster_state_30q_2x2_q1_index",
    metadata={
