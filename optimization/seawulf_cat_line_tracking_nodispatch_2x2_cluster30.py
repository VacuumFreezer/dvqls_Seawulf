...
from common.params_init_10qubits import init_angles_jax, init_norms_jax
...
def initialize_global_params_jax_light(system, n_qubits: int, layers: int, seed: int = 0):
...
def recover_global_solution(global_params, n_qubits):
    """
    Heavy helper for dense validation only.
    For 29 local qubits, each local state is a 2^29 complex vector.
    """
...
def _write_analysis(
    ...
    *,
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write("30-qubit cluster-state 2x2 distributed run\n")
        ...
        if dense_metrics_active and A_global is not None and global_b is not None and true_sol is not None:
            ...
            def get_state(weights):
                qml.BasicEntanglerLayers(weights=weights, wires=range(len(data_wires)), rotation=qml.RY)
                return qml.state()
...
def main():
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Force-enable sol_diff and ||Ax-b|| using dense state recovery when the loaded system supports it.",
    )
    ap.add_argument(
    ...
    GLOBAL_PARAMS, _ = initialize_global_params_jax_light(
