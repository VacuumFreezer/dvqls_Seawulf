...
from common.gradient_mixing import build_metropolis_matrix, consensus_mix_metropolis, consensus_mix_gradients_metropolis
...
def main():
    ...
    opt_gd   = optax.adam(lr_schedule)                 # beta/lambda
    ...
    for ep in range(1, args.epochs + 1):
        # -------------------------------------------------------
        # 1. Consensus on Parameters (Params Mixing)
        #    x(t+1/2) = W x(t)
        # -------------------------------------------------------
        GLOBAL_PARAMS = consensus_mix_metropolis(GLOBAL_PARAMS, Wm)

        # -------------------------------------------------------
        # 2. Compute Gradients at current mixed parameters
        # -------------------------------------------------------
        ...
        # -------------------------------------------------------
        # 3. Consensus on Gradients (Gradient Mixing)
        #    g_mixed = W g_local
        # -------------------------------------------------------
        # 这里替代了原本的 update_gradient_tracker
        mixed_grad_grid = consensus_mix_gradients_metropolis(
            current_grads=current_grad_grid,
            W=Wm
        )

        # -------------------------------------------------------
        # 4. Apply Gradients (Optimizer Step)
        #    x(t+1) = Optimizer(x(t+1/2), g_mixed)
        # -------------------------------------------------------
        
        # === Group Z: alpha/sigma -> Adam ===
        adam_keys = ["alpha", "sigma"]
        
        # 注意：这里传入的是 mixed_grad_grid (混合后的梯度)，而不是 tracker
        flat_grads_adam = to_jax_flat(flatten_params(mixed_grad_grid, keys=adam_keys))
        flat_params_adam = to_jax_flat(flatten_params(GLOBAL_PARAMS, keys=adam_keys))
        
        # JAX Optax Update: update(grads, state, params)
        adam_updates, opt_adam_state = opt_adam.update(flat_grads_adam, opt_adam_state, params=flat_params_adam)
        ...
        # beta/lambda 在 mixed_grad_grid 中是未混合的 (local)，直接使用
        flat_grads_gd  = to_jax_flat(flatten_params(mixed_grad_grid, keys=gd_keys))
        flat_params_gd = to_jax_flat(flatten_params(GLOBAL_PARAMS, keys=gd_keys))
        
        gd_updates, opt_gd_state = opt_gd.update(flat_grads_gd, opt_gd_state, params=flat_params_gd)
