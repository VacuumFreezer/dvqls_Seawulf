...
import objective.builder_cat as ib
    ...
    def get_local_state_segment(weights):
        ...
        for layer in range(layers):
            for q in range(n_qubits):
                qml.RY(weights[layer, q], wires=wires[q])
...
def main():
    ...
    # ---- init params ----
    GLOBAL_PARAMS, key = initialize_global_params_jax(SYSTEM, n_qubits=n_qubits, layers=LAYERS, seed=args.seed)
    ...
    grad_list = compute_grad(flatten_params(GLOBAL_PARAMS))
    ...
    for ep in range(1, args.epochs + 1):
        ...
        grads_flat = compute_grad(new_flat_params)
        current_cost = compute_loss(new_flat_params)
