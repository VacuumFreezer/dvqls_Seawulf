...
BENCHMARK_MEM = {f"B{idx}": "64G" for idx in range(1, 10)}
...
def build_task_command(
    *,
    ...
    cmd = [
        python_bin,
        "Graph_comparison/Ising_5qubits_9benchmark/seawulf_graph_comparison_ising_qjit.py",
        "--static_ops",
...
def main() -> None:
    ...
    run_root = ROOT / "Graph_comparison" / "Ising_5qubits_9benchmark" / "run"
    ...
    suite_tag = args.tag.strip() or (
        f"{timestamp}_graph_compare_ising5q_3x3_e{int(args.epochs)}"
    ...
    for benchmark in iter_benchmarks():
        ...
        sbatch_parts = [
            "sbatch",
            "--job-name",
            f"ising5q9_{benchmark.benchmark_id}",
            "--array",
            f"0-{len(seeds) - 1}",
            "--mem",
            ...
            "24:00:00",
            "--output",
            f"Graph_comparison/Ising_5qubits_9benchmark/run/{suite_tag}/slurm/%x_%A_%a.out",
            "--error",
            f"Graph_comparison/Ising_5qubits_9benchmark/run/{suite_tag}/slurm/%x_%A_%a.err",
            "--export",
            f"ALL,TAG={suite_tag}/benchmark={benchmark.benchmark_id}_{benchmark.name}",
            "Graph_comparison/Ising_5qubits_9benchmark/submit_array.slurm",
        ]
