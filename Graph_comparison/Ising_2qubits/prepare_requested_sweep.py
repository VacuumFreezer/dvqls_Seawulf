...
from Graph_comparison.Ising_5qubits_9benchmark.topology_registry_3x3 import iter_benchmarks


BENCHMARK_MEM = {f"B{idx}": "16G" for idx in range(1, 10)}
...
def build_task_command(
    *,
    ...
    cmd = [
        python_bin,
        "Graph_comparison/Ising_2qubits/seawulf_graph_comparison_ising_qjit.py",
        "--static_ops",
        "Graph_comparison/Ising_2qubits/static_ops_ising_4x4_2q.py",
        "--benchmark",
        benchmark_id,
        "--epochs",
        str(int(epochs)),
...
def main() -> None:
    ...
    run_root = ROOT / "Graph_comparison" / "Ising_2qubits" / "run"
    ...
    suite_tag = args.tag.strip() or (
        f"{timestamp}_graph_compare_ising2q_3x3_e{int(args.epochs)}"
    ...
    manifest = {
        ...
        "hyperparameters": {
            ...
            "workflow": "4x4 graph comparison on the 4-qubit Ising benchmark with 2 local qubits per agent",
        },
        "benchmarks": [],
    }
    ...
    for benchmark in iter_benchmarks():
        ...
        sbatch_parts = [
            "sbatch",
            "--job-name",
            f"ising2q9_{benchmark.benchmark_id}",
            "--array",
            f"0-{len(seeds) - 1}",
            "--mem",
            ...
            "24:00:00",
            "--output",
            f"Graph_comparison/Ising_2qubits/run/{suite_tag}/slurm/%x_%A_%a.out",
            "--error",
            f"Graph_comparison/Ising_2qubits/run/{suite_tag}/slurm/%x_%A_%a.err",
            "--export",
            f"ALL,TAG={suite_tag}/benchmark={benchmark.benchmark_id}_{benchmark.name}",
            "Graph_comparison/Ising_2qubits/submit_array.slurm",
        ]
