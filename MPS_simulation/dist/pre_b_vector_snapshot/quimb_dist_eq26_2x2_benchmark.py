...
CEN_DIR = THIS_DIR.parent.parent / "cen"
...
def to_jax_problem(problem_np: dict[str, object]):
    ...
    return {
        "blocks": tuple(blocks),
        "b_state": b_state,
        "b_scale": jnp.asarray(problem_np["b_scale"], dtype=jnp.float64),
...
def global_cost_jax(alpha, beta, cfg: Config, problem: dict[str, object]):
    ...
    b_state = problem["b_state"]
    b_scale = problem["b_scale"]
    ...
    for i in range(2):
        ...
        b_z_overlaps = [None, None]
        for k in range(2):
            b_z_overlaps[k] = jnp.real(b_state.overlap(z_states[k]))
            for p in range(2):
        ...
        for j in range(2):
            ...
            for k in range(2):
                ...
                b_z_term = b_z_term + lk * z_scales[k] * b_z_overlaps[k]
                for p in range(2):
                    zz_term = zz_term + (
                        lk
                        * laplacian[j, p]
...
def global_cost_numpy(alpha: np.ndarray, beta: np.ndarray, cfg: Config, problem: dict[str, object]) -> float:
    ...
    for i in range(2):
        ...
        b_z_overlaps = [None, None]
        for k in range(2):
            b_z_overlaps[k] = float(np.real(b_state.overlap(z_states[k])))
            for p in range(2):
        ...
        for j in range(2):
            ...
            for k in range(2):
                ...
                b_z_term += lk * z_scales[k] * b_z_overlaps[k]
                for p in range(2):
