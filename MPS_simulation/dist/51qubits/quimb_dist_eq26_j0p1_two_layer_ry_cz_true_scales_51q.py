...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the 51-qubit distributed J=0.1 study with an exact "
            "Hadamard -> (random Ry, CZ) -> (random Ry, CZ) ansatz and true-scale sigma/lambda init."
        )
    )
    parser.add_argument("--global-qubits", type=int, default=51)
    parser.add_argument("--local-qubits", type=int, default=50)
    ...
    parser.add_argument("--angle-init-radius", type=float, default=math.pi / 6.0)
    parser.add_argument(
        "--out-json",
        type=str,
        default=None,
...
def resolve_output_paths(cfg: Config) -> dict[str, Path]:
    ...
    default_base = run_dir / (
        f"j0p1_two_layer_ry_cz_true_scales_51q_pi6_"
...
def build_direct_problem(cfg: Config) -> dict[str, object]:
    ...
    return {
        ...
        "block_formula": {
            ...
            "A21": "I / zeta",
            "A22": a22_formula,
            "b_ij": f"(1 / (2 sqrt(2))) |+^{{{cfg.local_qubits}}}>",
        },
    }
...
def parameter_consensus_metrics(alpha: np.ndarray) -> dict[str, object]:
    variances = []
    l2_values = []
    for col in range(2):
        pair = np.asarray(alpha[:, col, :], dtype=np.float64)
        mean = np.mean(pair, axis=0, keepdims=True)
        variances.append(float(np.mean((pair - mean) ** 2)))
        l2_values.append(float(np.linalg.norm(pair[0] - pair[1])))
    return {
        "parameter_consensus_variance": float(np.mean(variances)),
        "parameter_consensus_variance_by_column": variances,
        "parameter_consensus_l2": float(math.sqrt(sum(v * v for v in l2_values))),
        "parameter_consensus_l2_by_column": l2_values,
    }
...
def build_final_diagnostics(alpha: np.ndarray, beta: np.ndarray, cfg: Config, problem_np: dict[str, object]) -> dict[str, object]:
    ...
    consensus = parameter_consensus_metrics(alpha)
    ...
    return {
        ...
        "parameter_consensus_variance": consensus["parameter_consensus_variance"],
        "parameter_consensus_variance_by_column": consensus["parameter_consensus_variance_by_column"],
        "parameter_consensus_l2": consensus["parameter_consensus_l2"],
        "parameter_consensus_l2_by_column": consensus["parameter_consensus_l2_by_column"],
...
def plot_cost_history(history: list[dict[str, float | int]], figure_path: Path) -> None:
    ...
    costs = np.maximum([float(item["global_cost"]) for item in logged], 1.0e-16)
    residuals = np.maximum([float(item.get("global_residual", np.nan)) for item in logged], 1.0e-16)
    consensus = np.maximum(
        [float(item.get("parameter_consensus_variance", np.nan)) for item in logged],
        1.0e-16,
    )

    fig, axes = plt.subplots(3, 1, figsize=(8.4, 10.2), dpi=160, sharex=True)
    axes[0].plot(iterations, costs, linewidth=1.8, color="#005f73")
    axes[0].set_ylabel("Global cost")
    axes[0].set_title("J=0.1 Two-Layer Ry+CZ Exact-Circuit Optimization")
    axes[0].set_yscale("log")
    axes[0].grid(True, alpha=0.25, linewidth=0.8)

    axes[1].plot(iterations, residuals, linewidth=1.8, color="#bb3e03")
    axes[1].set_ylabel("||Ax - b||")
    axes[1].set_yscale("log")
    axes[1].grid(True, alpha=0.25, linewidth=0.8)

    axes[2].plot(iterations, consensus, linewidth=1.8, color="#3a86ff")
    axes[2].set_xlabel("Iteration")
    axes[2].set_ylabel("Param var")
    axes[2].set_yscale("log")
    axes[2].grid(True, alpha=0.25, linewidth=0.8)
...
def write_report(report_path: Path, result: dict[str, object]) -> None:
    ...
    lines = [
        ...
        f"- Random angle init range: `[-{result['config']['angle_init_radius']}, +{result['config']['angle_init_radius']}]`.",
        ...
        f"- History metrics logged every `{result['config']['report_every']}` iterations.",
        "- Parameter consensus is computed from the local `x` parameter vectors `[sigma, alpha_angles]`, compared between the two rows for each column.",
        "",
        "## Outcome",
        ...
        f"- Final parameter consensus variance: `{diag['parameter_consensus_variance']:.12g}`.",
        f"- Final parameter consensus L2: `{diag['parameter_consensus_l2']:.12g}`.",
        f"- Per-column parameter consensus variance: `{diag['parameter_consensus_variance_by_column']}`.",
        f"- Per-column parameter consensus L2: `{diag['parameter_consensus_l2_by_column']}`.",
...
def main() -> None:
    ...
    for iteration in range(1, cfg.iterations + 1):
        ...
        if (iteration % cfg.report_every == 0) or (iteration == cfg.iterations):
            checkpoint_diag = build_final_diagnostics(
                np.asarray(state["alpha"]),
                np.asarray(state["beta"]),
                cfg,
                problem_np,
            )
            entry["global_residual"] = float(checkpoint_diag["global_residual"])
            entry["parameter_consensus_variance"] = float(checkpoint_diag["parameter_consensus_variance"])
            entry["parameter_consensus_l2"] = float(checkpoint_diag["parameter_consensus_l2"])
        ...
        if (iteration % cfg.report_every == 0) or (iteration == cfg.iterations):
            print(
                ...
                f"residual={entry['global_residual']:.6f} "
                f"param_var={entry['parameter_consensus_variance']:.6e} "
            ...
            write_progress_artifacts(
                ...
                state,
                start_time,
                final_diagnostics=checkpoint_diag,
            )
