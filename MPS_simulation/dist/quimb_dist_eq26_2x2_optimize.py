...
def agent_block_vector(alpha_agent: np.ndarray, cfg: Config) -> np.ndarray:
    ...
    return sigma * agent_unit_vector(alpha_agent, cfg)


def agent_unit_vector(alpha_agent: np.ndarray, cfg: Config) -> np.ndarray:
    state = build_circuit_numpy(cfg.local_qubits, alpha_agent[1:], cfg).psi
    return np.asarray(state.to_dense(), dtype=np.complex128).reshape(-1)
...
def build_final_diagnostics(
    ...
    x_block_norms = np.zeros((2, 2), dtype=np.float64)
    for i in range(2):
        row_units = []
        row_blocks = []
        for j in range(2):
            x_unit = agent_unit_vector(alpha[i, j], cfg)
            x_block = float(alpha[i, j, 0]) * x_unit
            row_units.append(x_unit)
            row_blocks.append(x_block)
            x_unit_norms[i, j] = float(np.linalg.norm(x_unit))
            x_block_norms[i, j] = float(np.linalg.norm(x_block))
        x_unit_vectors.append(row_units)
        x_block_vectors.append(row_blocks)

    row_action_1 = a11 @ x_block_vectors[0][0] + a12 @ x_block_vectors[0][1]
    row_action_2 = a21 @ x_block_vectors[1][0] + a22 @ x_block_vectors[1][1]
    b_row_1 = np.asarray(problem_np["b_rows"][0], dtype=np.complex128)
    b_row_2 = np.asarray(problem_np["b_rows"][1], dtype=np.complex128)
    row_copy_1 = np.concatenate(x_block_vectors[0])
    row_copy_2 = np.concatenate(x_block_vectors[1])
    column_block_1 = 0.5 * (x_block_vectors[0][0] + x_block_vectors[1][0])
    column_block_2 = 0.5 * (x_block_vectors[0][1] + x_block_vectors[1][1])

    return {
        "x_unit_state_norms": x_unit_norms.tolist(),
        "x_block_norms": x_block_norms.tolist(),
        "x_unit_vectors": [
            [encode_array(x_unit_vectors[i][j]) for j in range(2)]
            for i in range(2)
        ],
        "x_block_vectors": [
            [encode_array(x_block_vectors[i][j]) for j in range(2)]
            for i in range(2)
        ],
        "column_block_norms": [
            float(np.linalg.norm(column_block_1)),
            float(np.linalg.norm(column_block_2)),
        ],
        "row_copy_norms": [
            float(np.linalg.norm(row_copy_1)),
            float(np.linalg.norm(row_copy_2)),
        ],
        "row_actions": [
            encode_array(row_action_1),
            encode_array(row_action_2),
        ],
        "row_action_residual_norms": [
            float(np.linalg.norm(row_action_1 - b_row_1)),
            float(np.linalg.norm(row_action_2 - b_row_2)),
        ],
    }
...
def write_report(report_path: Path, result: dict[str, object]) -> None:
    ...
    sigma_final = np.asarray(alpha_final[:, :, 0], dtype=np.float64)
    lambda_final = np.asarray(beta_final[:, :, 0], dtype=np.float64)
    ...
    b_rows = [decode_array(item) for item in result["problem"]["b_rows"]]
    b_vectors = [
        [decode_array(item) for item in row]
        for row in result["problem"]["b_vectors"]
    ]
    row_actions = [decode_array(item) for item in result["final_diagnostics"]["row_actions"]]
    ...
    lines = [
        ...
        f"- Row-action residual norms `[||sum_j A_1j x_1j - b_1||_2, ||sum_j A_2j x_2j - b_2||_2]`: `{result['final_diagnostics']['row_action_residual_norms']}`",
        "",
        "## sigma and lambda",
        "### sigma",
        "```text",
        format_array(sigma_final),
        "```",
        "",
        "### lambda",
        "```text",
        format_array(lambda_final),
        "```",
        "",
        "## b decomposition",
        ...
        "### b_1",
        "```text",
        format_array(b_rows[0]),
        "```",
        "",
        "### b_2",
        "```text",
        format_array(b_rows[1]),
        "```",
        "",
        "### b_11",
        "```text",
        format_array(b_vectors[0][0]),
        "```",
        "",
        "### b_12",
        "```text",
        format_array(b_vectors[0][1]),
        "```",
        "",
        "### b_21",
        "```text",
        format_array(b_vectors[1][0]),
        "```",
        "",
        "### b_22",
        "```text",
        format_array(b_vectors[1][1]),
        "```",
        "",
        "## Reconstructed Row Actions",
        "### sum_j A_1j x_1j",
        "```text",
        format_array(row_actions[0]),
        "```",
        "",
        "### sum_j A_2j x_2j",
        "```text",
        format_array(row_actions[1]),
        "```",
        "",
    ...
    if result["initialization"]["mode"] == "random_uniform":
        lines.insert(5, f"- Initialization seed: `{result['initialization']['seed']}`")
