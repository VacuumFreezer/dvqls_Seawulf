...
except Exception:  # pragma: no cover - verifier/build can run without PennyLane
    qml = None
...
def beta_from_epsilon(epsilon: float) -> float:
    ...
    disc = (b * b) - (4.0 * a * c)
    if disc <= 0.0:
        raise ValueError(f"epsilon={eps} is too large for the chosen spectral target.")
    ...
    for beta in roots:
        if beta <= 0.0:
            continue
        residual = (25.0 * beta) + np.sqrt((beta * beta) + (eps * eps))
        if abs(residual - (1.0 - TARGET_MIN_EIGENVALUE)) < 1e-9:
            candidates.append(float(beta))
    if not candidates:
        raise ValueError(f"Could not determine a valid beta for epsilon={eps}.")
    return min(candidates)


def alpha_from_beta(beta: float) -> float:
    return 1.0 - (13.0 * float(beta))
...
def _reference_angles(n_local_qubits: int, boundary_flip: bool) -> np.ndarray:
    angles = np.full((int(n_local_qubits),), BASE_BLOCK_ANGLE, dtype=np.float64)
    if boundary_flip and int(n_local_qubits) > 0:
        angles[0] = -angles[0]
    return angles
...
def _apply_cz_phase_dense(state: np.ndarray, n_qubits: int, edge: Tuple[int, int]) -> np.ndarray:
    ...
    right_phase = np.asarray([0.0, 1.0], dtype=np.float64).reshape(
        (1,) * right + (2,) + (1,) * (int(n_qubits) - right - 1)
    )
...
def reference_state_vector(
    ...
    *,
    ...
    angles = _reference_angles(int(n_local_qubits), boundary_flip=boundary_flip)
...
def _apply_reference_block(
    ...
    *,
    ...
    angles = _reference_angles(len(data_wires), boundary_flip=boundary_flip)
...
def _global_cluster_state_vector() -> np.ndarray:
    odd_edges, even_edges, _ = _local_chain_edge_layers(0)
    return reference_state_vector(
        N_TOTAL_QUBITS,
        odd_edges=odd_edges,
        even_edges=even_edges,
        boundary_flip=False,
        global_phase_sign=1.0,
    )


def _project_prefix_block(global_state: np.ndarray, prefix_bits: Sequence[int], index_qubits: int) -> np.ndarray:
    tensor = np.asarray(global_state, dtype=complex).reshape((2,) * N_TOTAL_QUBITS)
    slices = tuple(int(bit) for bit in prefix_bits) + (slice(None),) * (N_TOTAL_QUBITS - int(index_qubits))
    return np.asarray(tensor[slices], dtype=complex).reshape(-1)
...
class LinearSystemData:
    ...
    def get_b_vectors(self, sys_id: int):
        ...
        if row in self._block_b_cache:
            ...
            b_total = float(self.metadata["block_norm"]) * block_state
            self._block_b_cache[row] = b_total
        individual = []
        row_weight_sum = float(sum(self.b_weights[row]))
        for agent_id in range(self.n):
            weight = float(self.b_weights[row][agent_id])
            individual.append((weight / row_weight_sum) * b_total)
    ...
    def true_solution_vector(self) -> np.ndarray:
        ...
        if SCIPY_AVAILABLE:
            ...
            return self._true_solution_cache
        self._true_solution_cache = np.asarray(np.linalg.solve(self.get_global_matrix(), global_b), dtype=np.complex128)
        return self._true_solution_cache
...
def build_partition_namespace(index_qubits: int, *, epsilon: float = DEFAULT_EPSILON) -> dict:
    ...
    global_terms.append((epsilon, {13: "X"}, "X_13"))
    ...
    r = float(np.sqrt((beta * beta) + (epsilon * epsilon)))
    distinct_eigenvalues = sorted({alpha + (beta * s) + sign * r for s in range(-12, 13, 2) for sign in (-1.0, 1.0)})

    global_b_dense = _global_cluster_state_vector()
    prefix_state_max_abs_diff = 0.0
    prefix_norm_max_abs_diff = 0.0
    for row_id in range(n_agents):
        bits = bitstring_tuple(row_id, k)
        projected = _project_prefix_block(global_b_dense, bits, k)
        projected_norm = float(np.linalg.norm(projected))
        reference = block_norm * reference_state_vector(
            n_data_qubits,
            odd_edges=odd_edges_local,
            even_edges=even_edges_local,
            boundary_flip=bool(boundary_flip_by_row[row_id]),
            global_phase_sign=float(block_phase_by_row[row_id]),
        )
        prefix_state_max_abs_diff = max(
            prefix_state_max_abs_diff,
            float(np.max(np.abs(projected - reference))),
        )
        prefix_norm_max_abs_diff = max(prefix_norm_max_abs_diff, abs(projected_norm - block_norm))

    spectrum_info = {
        "lambda_min": float(min(distinct_eigenvalues)),
        "lambda_max": float(max(distinct_eigenvalues)),
        "condition_number": float(max(distinct_eigenvalues) / min(distinct_eigenvalues)),
        "n_distinct_eigenvalues": len(distinct_eigenvalues),
        "distinct_eigenvalues": distinct_eigenvalues,
        "b_is_exact_solution_at_epsilon_zero": True,
    }

    metadata = {
        "benchmark_name": "13q_full_cluster_stabilizer_prefix_partition",
        ...
        "real_problem": True,
        "recommended_ansatz": "paper_fig3_ry_cz",
        "recommended_layers": 3,
        "init_sigma_target": 1.0,
        "init_angle_fill": BASE_BLOCK_ANGLE,
        "agent_init_overrides": agent_init_overrides,
        "exact_solution_relation": "Computed numerically from the reconstructed sparse global matrix.",
        "reference_row_gates": b_gate_by_row,
        "b_projection_verification_max_abs_diff": prefix_state_max_abs_diff,
        "b_projection_norm_max_abs_diff": prefix_norm_max_abs_diff,
    }
    ...
    system = LinearSystemData(
        ...
        name=f"full_cluster_stabilizer13_prefix_partition_{partition_label}",
        metadata=metadata,
