...
PARTITIONS = [
    {
        "name": "8d8",
        "static_ops": "Partition_comparison_qjit/New_stabilizer/8d8/static_ops_13q_xzx_fresh_8x8.py",
        "mem": "128G",
        "nice": None,
    },
    {
        "name": "1d1",
        "static_ops": "Partition_comparison_qjit/New_stabilizer/1d1/static_ops_13q_xzx_fresh_1x1.py",
        "mem": "16G",
        "nice": 0,
    },
    {
        "name": "2d2",
        "static_ops": "Partition_comparison_qjit/New_stabilizer/2d2/static_ops_13q_xzx_fresh_2x2.py",
        "mem": "16G",
        "nice": 1000,
    },
    {
        "name": "4d4",
        "static_ops": "Partition_comparison_qjit/New_stabilizer/4d4/static_ops_13q_xzx_fresh_4x4.py",
        "mem": "128G",
        "nice": 2000,
    },
]
...
def build_task_command(
    *,
    ...
    cmd = [
        python_bin,
        "Partition_comparison_qjit/New_stabilizer/seawulf_partition_comparison_qjit.py",
        "--static_ops",
        static_ops,
        "--topology",
        "line",
...
def main() -> None:
    ...
    run_root = root / "Partition_comparison_qjit" / "New_stabilizer" / "run"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    suite_tag = args.tag.strip() or (
        f"xzx_jit5_{args.init_mode}_siglam1_lr5e3_nodecay_e{int(args.epochs)}_log{int(args.log_every)}_s{len(args.seeds)}_{timestamp}"
    )
    ...
    for partition in PARTITIONS:
        sbatch_parts = [
            ...
            "24:00:00",
            "--output",
            f"Partition_comparison_qjit/New_stabilizer/run/{suite_tag}/slurm/%x_%A_%a.out",
            "--error",
            f"Partition_comparison_qjit/New_stabilizer/run/{suite_tag}/slurm/%x_%A_%a.err",
        ]
        ...
        sbatch_parts.extend(
            [
                ...
                "Partition_comparison_qjit/New_stabilizer/submit_array.slurm",
            ]
        )
        submit_lines.append(" ".join(shlex.quote(part) for part in sbatch_parts))
        if partition["name"] == "8d8" and int(args.stagger_seconds) > 0:
